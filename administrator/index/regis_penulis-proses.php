...
if(isset($_POST['daftar'])){
	//AUTONUMBER START
	$query = "select MAX(RIGHT(kd_pen,4)) as max_id from penulis ORDER BY kd_pen";
  ...
  $tentang_saya = mysql_real_escape_string($_POST['tentang_saya']);
	$password = mysql_real_escape_string(md5($_POST['password']));
	$konpass = mysql_real_escape_string(md5($_POST['konf_pass']));
			...
			//if(!empty($lokasi_file)){
				...
				//if($tipe_file == "image/jpeg" || $tipe_file == "image/jpg" ||$tipe_file == "image/png"){
                                   //if($ukuran_gambar <= $ukuran){
                                        ...
                                        $input = mysql_query("INSERT INTO penulis VALUES('Pen$idfix', '$username', '$nama', '$tmp_lahir', '$tgl_lahir', '$email','$alamat', '$no_hand','$jenkel','$tentang_saya','$password','$konpass', NOW())") or die(mysql_error());
